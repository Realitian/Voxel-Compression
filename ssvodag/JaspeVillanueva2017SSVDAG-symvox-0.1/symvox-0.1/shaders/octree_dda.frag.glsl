...
float getMinT(in const int delta = 8) {
...
void main() {
	...
	if(useMinDepthTex) {
		...
		color = vec3(getMinT() / length(sceneBBoxMax-sceneBBoxMin));
		return;
	}
#endif
	vec2 t_min_max = vec2(useMinDepthTex ? getMinT() : 0, 1e30);
